...
class SimpleDB:
    ...
    def merge_chunks_bool(self, chunk_i, chunk_j, join_conditions):
        ...
        # table1_column, operator, table2_column = join_conditions
        # return self.operator_mapping[operator](chunk_i[table1_column], chunk_j[table2_column])
    ...
    def execute_join_query(self, table_name, column_list=None,conditions=None, order_by=None, join_table=None, join_conditions=None):
        ...
        if join_table:
            ...
            for file_number_i in range(table_data['no_of_files']):
                for join_file_number_j in range(join_table_data['no_of_files']):
                    ...
                    for row_i in self._load_csv_rows_in_a_file(table_name, file_number_i):
                        for row_j in self._load_csv_rows_in_a_file(join_table, join_file_number_j):
                            # Filtering
            ...
            if result:
                # table_column_names.extend(join_table_column_names)
                # column_dict[table_name] = table_column_names
        ...
        # merge all the files
        if order_by:
            self.merge_sort_csv_files([f"join_{i}.csv" for i in range(file_chunks)], f"join_{table_name}_{join_table}.csv", order_by)
    ...
    def merge_sort_csv_files(self,file_list, output_filepath, order_by):
        """
        Merges multiple sorted CSV files into a single sorted CSV file,
        by merging two files at a time.
        """

        def merge_two_csv_files(file1, file2, output_file, col, order):
            """
        ...
        while len(file_list) > 1:
            ...
            for i in range(0, len(file_list), 2):
                if i + 1 < len(file_list):
                    ...
                    merge_two_csv_files(file_list[i], file_list[i + 1], output_file, col=order_by[0], order=order_by[1])
